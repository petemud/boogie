...
var {:layer 0,1} snapshots: [ChannelPiece]Snapshot;
var {:linear} {:layer 0,2} contents: Map ChannelPiece Snapshot;
...
atomic action {:layer 1,2} SendFirstAction({:linear_in} one_s: One ChannelPiece, snapshot: Snapshot)
modifies contents;
{
...
right action {:layer 1,2} ReceiveFirstAction({:linear} one_r: One ChannelPiece) returns ({:linear} one_s: One ChannelPiece, snapshot: Snapshot)
modifies contents;
{
    var {:linear} cell: Cell ChannelPiece Snapshot;
    var s: ChannelPiece;
...
refines ReceiveFirstAction;

atomic action {:layer 1,2} SendSecondAction({:linear_in} one_s: One ChannelPiece, snapshot: Snapshot)
modifies contents;
{
    var {:linear} cell: Cell ChannelPiece Snapshot;
...
refines main_f'';
{
    ...
    call SendFirstAction(one_s, snapshot);
}
...
requires IsSendFirst(perm->val->val) && IsValidMemIndexPiece(perm->val);
{
    ...
    if (Set_IsSubset(AllMemIndexPieces(s), pset)) {
        ...
        call SendFirstAction(one_s, snapshots[s]);
    }
}
...
modifies contents;
{
    ...
    one_s := One(s);
    call Fractions_To_One(one_s, MemIndices(), sps);
    call SendSecondAction(one_s, snapshot);
}
...
requires IsSendSecond(perm->val->val) && IsValidMemIndexPiece(perm->val);
{
    ...
    if (Set_IsSubset(AllMemIndexPieces(s), pset)) {
        ...
        one_s := One(s);
        call Fractions_To_One(one_s, MemIndices(), pieces);
        call SendSecondAction(one_s, snapshots[s]);
    }
}
